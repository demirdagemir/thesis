...
from Aion.data_inference.extraction.featureExtraction import extractDroidmonFeatures
from Aion.utils.graphics import prettyPrint
...
class Explorer(object):
    ...
    def filter_logcat_for_droidmon(self, logcat_file_param):
        ...
        log.info("Retrieving \"Droidmon-apimonitor-%s\" tags from log" % self.manifest.package_name)
        catlog = subprocess.Popen(("cat", logcat_file_param), stdout=subprocess.PIPE)
        try:
            output = subprocess.check_output(("grep", "-i", "droidmon-apimonitor-%s" % self.manifest.package_name),
                                             stdin=catlog.stdout)
        except subprocess.CalledProcessError as cpe:
            log.info("Could not find the tag \"droidmon-apimonitor-%s in the logs" % self.manifest.package_name,
                        "warning")
            return True
    ...
    def process_apk(self):
        ...
        log.info("Clearing device log before test")
        max_runs = int(self.config["branchexp"]["max_runs"])
        log.info("max run is set to %s" % max_runs)
    ...
    def _get_exec_path_infos(self, alert):
        ...
        path_infos = get_path_info_for_alert(path_finder_acfg, alert)
    ...
    def run_apk(self):
        ...
        logcatFileParam = self.output_dir + "/dumped_logcat_" + str(self.run_id) + ".log"
        self.dump_logcat(logcatFileParam)
        self.filter_logcat_for_droidmon(logcatFileParam)

        prettyPrint("---------- Extracting Droidmon Features -----------------", "warning")
        droidmonFeatures = extractDroidmonFeatures("%s/dumped_logcat_%s.log_filtered.log" % (self.output_dir, str(self.run_id)))
        with open(self.output_dir + "/extractedDM_" + str(self.run_id) + "_traces.log", "w") as fp:
            fp.write('\n'.join('%s' % x for x in droidmonFeatures[0]))
        with open(self.output_dir + "/extractedDM_" + str(self.run_id) + "_features.log", "w") as fp:
