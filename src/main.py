...
from Aion.shared.DroidbotTest import *
from Aion.shared.DroidutanTest import *
...
import hashlib  # Used for analysis of introspy generated databases
import os, sys, glob, shutil, argparse, subprocess, sqlite3, time, threading, pickledb, random

# Jona's Phenax
from Phenax.main_mac import phenax


#requiredParams: malwaredir, goodwareDir(?), dataset, runnumber

# TODO: Get rid of arguments that is not needed
# ------------------------------------------- #
...
def defineArguments():
    ...
    parser.add_argument("-x", "--malwaredir",
                        help="The directory containing the malicious APK's to analyze and use as training/validation dataset",
                        required=True)
    parser.add_argument("-g", "--goodwaredir",
                        help="The directory containing the benign APK's to analyze and use as training/validation dataset",
                        required=True)
    parser.add_argument("-d", "--datasetname",
                        help="A unique name to give to the dataset used in the experiment (for DB storage purposes)",
                        required=True)
    parser.add_argument("-r", "--runnumber",
                        help="The number of the current run of the experiment (for DB storage purposes)", required=True)
    parser.add_argument("-f", "--analyzeapks", help="Whether to perform analysis on the retrieved APK's",
                        required=False, default="no", choices=["yes", "no"])
    parser.add_argument("-t", "--analysistime", help="How long to run monkeyrunner (in seconds)", required=False,
                        default=60)
    parser.add_argument("-u", "--analysisengine", help="The stimulation/analysis engine to use", required=False,
                        choices=["droidbot", "droidutan", "groddroid"], default="groddroid")
    parser.add_argument("-v", "--vmnames",
                        help="The name(s) of the Genymotion machine(s) to use for analysis (comma-separated)",
                        required=False, default="")
    parser.add_argument("-z", "--vmsnapshots",
                        help="The name(s) of the snapshot(s) to restore before analyzing an APK (comma-separated)",
                        required=False, default="")
    parser.add_argument("-y", "--validation",
                        help="Instructs Aion how to perform validation i.e. on 'training' or 'validation' datasets",
                        required=True, choices=["training", "validation"])
...
def main():
    try:
        ...
        prettyPrint(
            "[GOODWARE] Training dataset size is %s, test dataset size is %s" % (len(goodTraining), len(goodTest)))
        ...
        while (round(currentMetrics["f1score"] - previousMetrics["f1score"], 2) >= -(
            ...
            aionDB.update("run", [("runIterations", str(iteration))], [("runID", arguments.runnumber), (
            "runDataset", arguments.datasetname)])  # UPDATE run SET runIterations=X WHERE runID=[runnumber]
            if arguments.analyzeapks == "yes":
                ...
                while len(allAPKs) > 0:
                    ...
                    if len(rows) < 1:
                        ...
                        aionDB.insert("app", ["appName", "appType", "appRunID", "appRuns"],
                                      [appName, appType, arguments.runnumber, 1])
                    else:
                    ...
                    while len(availableVMs) < 1:
                        ...
                        for p in currentProcesses:
                            if not p.is_alive():
                                if verboseON():
                                    prettyPrint("Process \"%s\" is dead. A new AVD is available for analysis" % p.name,
                                                "debug")
                    ...
                    if arguments.analysisengine == "droidutan":
                        ...
                        p = DroidbotAnalysis(pID, currentVM, currentVM, currentAPK,
                                             allSnapshots[allVMs.index(currentVM)], int(arguments.analysistime))
                    ################### GRODDDROID ######################################################
                    elif arguments.analysisengine == "groddroid":
                        # ---------- PHENAX ARGUMENTS --------
                        # python3 main_mac.py
                        # -explorer_dir "/Users/jonaneumeier/Developer/Uni/6. Semester/Bachelor-thesis/bachelor_code/grodd_new_version/BranchExplorer"
                        # -inputDir /Users/jonaneumeier/Dropbox/Uni/6.\ Semester/Bachelor-thesis/bachelor_code/test_data/input
                        # -outputDir /Users/jonaneumeier/Dropbox/Uni/6.\ Semester/Bachelor-thesis/bachelor_code/test_data/output
                        # parserCustom = argparse.ArgumentParser(prog="main.py",
                        #                                  description="This is the main program. TODO: explain the program here briefly")
                        # parserCustom.add_argument("-explorer_dir", "--branchExplorer_dir",
                        #                     help="The path to the subdirectory BranchExplorer in the GroddDroid files",
                        #                     default="/Users/emirdemirdag/Developer/PycharmProjects/Phenax/Phenax/grodd_new_version/BranchExplorer")
                        # parserCustom.add_argument("-inputDir", "--inputDirectory",
                        ...
                        #                     default="/Users/emirdemirdag/Developer/Phenax/apps")
                        # parserCustom.add_argument("-outputDir", "--outputDirectory",
                        #                     help="The path to the directory for the output",
                        #                     default="/Users/emirdemirdag/Developer/PycharmProjects/thesis/output")
                        # parserCustom.add_argument("-kf", "--kfold", help="Set the value of K for cross validation", default=2)
                        # parserCustom.add_argument("-selectBest", "--selectBest",
                        #                     help="Set the number of the features that should be selected >= 30",
                        #                     default=1)
                        # parserCustom.add_argument("-VMname", "--vm_name",
                        #                     help="The name or ID of the Virtual Box virtual machine to use",
                        #                     default="Google Nexus 5X - 7.0.0 - API 24 - 1080x1920")
                        # parserCustom.add_argument("-restore_snapshot", "--restore_snapshot",
                        #                     help="The name or ID of the Virtual Box snapshot to restore the virtual machine",
                        #                     default="Google Nexus 5X - 7.0.0 - API 24 - 1080x1920")
                        branchExplorerDir = "/Users/emirdemirdag/Developer/PycharmProjects/Phenax/Phenax/grodd_new_version/BranchExplorer"
                        #inputDirectory = "/Users/emirdemirdag/Developer/Phenax/apps"
                        inputDirectory = "/Users/emirdemirdag/Developer/PycharmProjects/thesis/input"
                        outputDirectory = "/Users/emirdemirdag/Developer/PycharmProjects/thesis/output"
                        ## TODO: Make it look like Droidutan
                        # print parserCustom
                        p = phenax(branchExplorerDir, inputDirectory, outputDirectory)
                    #####################################################################################

                        ## TODO: Emir: Make it a Process -- these are commented out from Alei's code.
                ...
                for app in allApps:
                    ...
                    inFile = app.replace(".apk",
                                         "_filtered.log") if arguments.analysisengine == "droidutan" else app.replace(
                        ".apk", "_droidbot/logcat_filtered.log")
                    # EMIR
                    print inFile
                    print "---------- !!!!!!!!! printend inFile !!!!!! --------"
                    ### EMIR end
                    ...
                    if arguments.featuretype == "dynamic" or arguments.featuretype == "hybrid":
                        ### ------ EXTRACT DROIDMON FEATURES ------------ searchword: jumpdroidmon ######
            ...
            allFeatureFiles = glob.glob("%s/*.%s" % (arguments.malwaredir, arguments.fileextension)) + glob.glob(
                "%s/*.%s" % (arguments.goodwaredir, arguments.fileextension))
            if len(allFeatureFiles) < 1:
                prettyPrint("Could not retrieve any feature files. Exiting", "error")
                return False

            allDynamicFiles = glob.glob("%s/*.dyn" % arguments.malwaredir) + glob.glob(
                "%s/*.dyn" % arguments.goodwaredir)
            ...
            for k in K:
                ...
                if arguments.featuretype == "hybrid" and len(allDynamicFiles) > 1:
                    ...
                    predicted, predicted_test = ScikitLearners.predictAndTestKNN(Xtrd, ytr, Xted, yte, K=k,
                                                                                 selectKBest=int(arguments.selectkbest))
                    metrics = ScikitLearners.calculateMetrics(ytr, predicted)
                    metrics_test = ScikitLearners.calculateMetrics(yte, predicted_test)
            ...
            for e in E:
                ...
                if arguments.featuretype == "hybrid" and len(allDynamicFiles) > 1:
                    ...
                    predicted, predicted_test = ScikitLearners.predictAndTestRandomForest(Xtrd, ytr, Xted, yte,
                                                                                          estimators=e, selectKBest=int(
                            arguments.selectkbest))
                    metrics = ScikitLearners.calculateMetrics(ytr, predicted)
                    metrics_test = ScikitLearners.calculateMetrics(yte, predicted_test)
            ...
            if arguments.featuretype == "hybrid" and len(allDynamicFiles) > 1:
                ...
                predicted, predicted_test = ScikitLearners.predictAndTestSVM(Xtrd, ytr, Xted, yte,
                                                                             selectKBest=int(arguments.selectkbest))
                metrics = ScikitLearners.calculateMetrics(ytr, predicted)
                metrics_test = ScikitLearners.calculateMetrics(yte, predicted_test)
            ...
            if arguments.featuretype == "hybrid" and len(allDynamicFiles) > 1:
                # Same for dynamic features
                predicted, predicted_test = ScikitLearners.predictAndTestEnsemble(Xtrd, ytr, Xted, yte,
            ...
            for m in metricsDict:
                ...
                aionDB.insert(table="datapoint",
                              columns=["dpLearner", "dpIteration", "dpRun", "dpTimestamp", "dpFeature", "dpType",
                                       "dpAccuracy", "dpRecall", "dpSpecificity", "dpPrecision", "dpFscore"],
                              values=[learnerID, str(iteration), arguments.runnumber, tstamp, arguments.featuretype,
                                      "TRAIN", str(metricsDict[m]["accuracy"]), str(metricsDict[m]["recall"]),
                                      str(metricsDict[m]["specificity"]), str(metricsDict[m]["precision"]),
                                      str(metricsDict[m]["f1score"])])
            ...
            if arguments.featuretype == "hybrid" and len(allDynamicFiles) > 1:
                for d in dynamicDict:
                    # The average metrics for training dataset
                    prettyPrint(
                        "Metrics using %s-fold cross validation and %s and dynamic features" % (arguments.kfold, d),
                        "output")
                    ...
                    aionDB.insert(table="datapoint",
                                  columns=["dpLearner", "dpIteration", "dpRun", "dpTimestamp", "dpFeature", "dpType",
                                           "dpAccuracy", "dpRecall", "dpSpecificity", "dpPrecision", "dpFscore"],
                                  values=[learnerID, str(iteration), arguments.runnumber, tstamp, "dynamic", "TRAIN",
                                          str(dynamicDict[d]["accuracy"]), str(dynamicDict[d]["recall"]),
                                          str(dynamicDict[d]["specificity"]), str(dynamicDict[d]["precision"]),
                                          str(dynamicDict[d]["f1score"])])
            ...
            for m in metricsDict_test:
                ...
                aionDB.insert(table="datapoint",
                              columns=["dpLearner", "dpIteration", "dpRun", "dpTimestamp", "dpFeature", "dpType",
                                       "dpAccuracy", "dpRecall", "dpSpecificity", "dpPrecision", "dpFscore"],
                              values=[learnerID, str(iteration), arguments.runnumber, tstamp, arguments.featuretype,
                                      "TEST", str(metricsDict_test[m]["accuracy"]), str(metricsDict_test[m]["recall"]),
                                      str(metricsDict_test[m]["specificity"]), str(metricsDict_test[m]["precision"]),
                                      str(metricsDict_test[m]["f1score"])])
            ...
            if arguments.featuretype == "hybrid" and len(allDynamicFiles) > 1:
                for d in dynamicDict_test:
                    ...
                    aionDB.insert(table="datapoint",
                                  columns=["dpLearner", "dpIteration", "dpRun", "dpTimestamp", "dpFeature", "dpType",
                                           "dpAccuracy", "dpRecall", "dpSpecificity", "dpPrecision", "dpFscore"],
                                  values=[learnerID, str(iteration), arguments.runnumber, tstamp, "dynamic", "TEST",
                                          str(dynamicDict_test[d]["accuracy"]), str(dynamicDict_test[d]["recall"]),
                                          str(dynamicDict_test[d]["specificity"]),
                                          str(dynamicDict_test[d]["precision"]), str(dynamicDict_test[d]["f1score"])])
        ...
        aionDB.update("run", [("runEnd", getTimestamp(includeDate=True))],
                      [("runID", arguments.runnumber)])  # UPDATE run SET runEnd=X WHERE runID=[runnumber]
